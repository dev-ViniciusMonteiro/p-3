...
app.get('/api/status', (req, res) => {
  res.status(200).json({ status: 'online', module: 'paciente' });
});
...
async function startServer() {
  try {
    ...
    // Iniciar servidor
    app.listen(PORT, () => {
      console.log(`Servidor do módulo de pacientes rodando na porta ${PORT}`);
      logger.info(`Servidor do módulo de pacientes iniciado na porta ${PORT}`);
    });
  } catch (error) {
